        ...
        if ([propertyType hasPrefix:@"T@"]) {
            ...
            if (propertys.count > 1) {
                ...
                for (NSString *protocolName in protocolNames) {
                    ...
                    if ([modelClass isSubclassOfClass:[JKDBModel class]]) {
...
inline NSArray *modelToData(JKDBModel *model, BOOL isAdd){
    ...
    for (NSUInteger i = 0, count = model.jdm_columeNames.count; i < count; i++) {
        ...
        if (pType == JDMPropertyReal || pType == JDMPropertyInteger || pType == JDMPropertyText) {
            ...
            JKDBModel *model = value;
            
            if (model && [model isKindOfClass:[JKDBModel class]]) {
            ...
            if (modelClass) {
                ...
                for (JKDBModel *model in value) {
                    
                    if (![model isKindOfClass:[JKDBModel class]]) {
                        NSLog(@"model is not JKDBModel!");
                        break;
                    }
...
inline NSArray *dataToModel(FMResultSet *resultSet, Class modelClass){
    ...
    while ([resultSet next]) {
        JKDBModel *model = [[modelClass alloc] init];
        JDMPropertyType *pTypes = (JDMPropertyType *)model.jdm_propertyTypes.bytes;
...
- (BOOL)deleteObject
{
    ...
    for (int i=0; i< self.jdm_columeNames.count; i++) {
        ...
        if (pType == JDMPropertyModel) {
            ...
            JKDBModel *subModel = [self valueForKey:columeName];
            
            if (subModel && [subModel isKindOfClass:[JKDBModel class]]) {
            ...
            for (JKDBModel *subModel in subModels) {
...
+ (void)findByPK:(long long)inPk callback:(void(^)(JKDBModel *model))callback{
    
    dispatch_async([self.class JD_Queue], ^{
        
        JKDBModel *model = [self findByPK:inPk];
...
+ (void)findFirstWithFormatAsync:(void(^)(JKDBModel *model))callback format:(NSString *)format, ...{
    ...
    dispatch_async([self.class JD_Queue], ^{
        ...
        JKDBModel *model = [self findFirstWithFormat:criteria];
...
+ (void)findFirstByCriteriaAsync:(NSString *)criteria callback:(void(^)(JKDBModel *model))callback{
    
    dispatch_async([self.class JD_Queue], ^{
        
        JKDBModel *model = [self findFirstByCriteria:criteria];
